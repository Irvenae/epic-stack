...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (intent === 'delete' || !image) {
    await db.delete(schema.userImage).where(eq(schema.userImage.userId, userId))
    return redirect('/settings/profile')
  }

  await db.transaction(async (trx) => {
    await trx
      .delete(schema.userImage)
      .where(eq(schema.userImage.userId, userId))
    await trx.insert(schema.userImage).values({ ...image, userId })
  })
