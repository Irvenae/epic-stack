...
async function userCanDeleteConnections(userId: string) {
  ...
  return Boolean(userConnectionsCount && userConnectionsCount > 1)
}

export async function loader({ request }: LoaderFunctionArgs) {
  const userId = await requireUserId(request)
  const timings = makeTimings('profile connections loader')
  const rawConnections = await db.query.connection.findMany({
    columns: {
      id: true,
      providerName: true,
      providerId: true,
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  await db
    ...
    .where(
      and(
        eq(schema.connection.id, connectionId),
