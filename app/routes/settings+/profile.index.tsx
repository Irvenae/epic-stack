...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const user = await db.query.user.findFirst({
    ...
    with: {
      ...
      sessions: {
        where: gt(schema.session.expiresAt, currentTime),
        columns: { id: true },
      },
    },
    columns: {
  ...
  const twoFactorVerification = await db.query.verification.findFirst({
    columns: { id: true },
    where: and(
      eq(schema.verification.target, userId),
  ...
  const password = await db.query.password.findFirst({
    columns: { userId: true },
    where: eq(schema.password.userId, userId),
  })
...
async function signOutOfSessionsAction({ request, userId }: ProfileActionArgs) {
  ...
  await db
    ...
    .where(
      and(eq(schema.session.userId, userId), ne(schema.session.id, sessionId)),
    )
...
function SignOutOfSessions() {
  ...
  const otherSessionsCount =
		data.user.sessions.length === 0 ? 0 : data.user.sessions.length - 1
  return (
    <div>
      {otherSessionsCount ? (
