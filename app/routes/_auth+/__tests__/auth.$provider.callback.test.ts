...
test('when a user is logged in, it creates the connection', async () => {
  ...
  const connection = await db.query.connection.findFirst({
    columns: { id: true },
    where: and(
      eq(schema.connection.userId, schema.session.userId),
      eq(schema.connection.providerId, githubUser.profile.id.toString()),
    ),
  })
  expect(
    connection,
...
test('if a user is not logged in, but the connection exists and they have enabled 2FA, send them to verify their 2FA and do not make a session', async () => {
  ...
  await db.insert(schema.verification).values({
    ...
    target: userId,
    expiresAt: Date.now() + hoursToMilliseconds(1),
    ...config,
  })
