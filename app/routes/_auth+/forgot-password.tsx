...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const submission = await parseWithZod(formData, {
    schema: ForgotPasswordSchema.superRefine(async (data, ctx) => {
      const user = await db.query.user.findFirst({
        columns: { id: true },
        where: or(
          eq(schema.user.email, data.usernameOrEmail),
          eq(schema.user.username, data.usernameOrEmail),
        ),
      })
      if (!user) {
        ctx.addIssue({
  ...
  const user = await db.query.user.findFirst({
    ...
    where: or(
      eq(schema.user.email, usernameOrEmail),
      eq(schema.user.username, usernameOrEmail),
    ),
  })
