...
export async function handleNewSession(
  {
    ...
    remember,
		...
		request: Request
		session: { userId: string; id: string; expiresAt: number }
		redirectTo?: string
		remember: boolean
	},
  ...
  const verification = await db.query.verification.findFirst({
    columns: { id: true },
    where: and(
      eq(schema.verification.target, session.userId),
      ...
      combineResponseInits(
        {
          headers: {
            'set-cookie': await authSessionStorage.commitSession(authSession, {
              expires: remember ? new Date(session.expiresAt) : undefined,
            }),
          },
        },
        responseInit,
...
export async function handleVerification({
  ...
  if (unverifiedSessionId) {
    const session = await db.query.session.findFirst({
      ...
      where: eq(schema.verification.id, unverifiedSessionId),
    })
    if (!session) {
