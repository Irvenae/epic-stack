...
async function main() {
  ...
  try {
    ...
    const permissions = await db
      ...
      .returning({ id: schema.permission.id, access: schema.permission.access })
    ...
    const permissionRoleMapping = permissions.map(({ id, access }) => {
      if (access === 'any') {
        return { permissionId: id, roleId: adminRoleId }
      }
      return { permissionId: id, roleId: userRoleId }
    })
    await db.insert(schema.permissionToRole).values(permissionRoleMapping)
    ...
    for (let index = 0; index < totalUsers; index++) {
      ...
      await db
        .insert(schema.password)
        .values({ ...createPassword(userData.username), userId })
      await db
        .insert(schema.userImage)
        .values({ ...userImages[index % userImages.length]!, userId })
      await db.insert(schema.roleToUser).values({ userId, roleId: userRoleId })
      const nrEntries = faker.number.int({ min: 1, max: 3 })
      const notesToCreate = Array.from({
        length: nrEntries,
      }).map(() => ({
      ...
      const noteImagesToCreate = Array.from({
        length: nrEntries,
      }).map((_, idx) => {
        ...
        return { ...img, noteId: noteIds[idx]! }
      })
      console.log(noteImagesToCreate)
      await db.insert(schema.noteImage).values(noteImagesToCreate)
    }
    ...
    const userId = (
			await db
			  .insert(schema.user)
			  .values({ email: 'kody@kcd.dev', username: 'kody', name: 'Kody' })
			  .returning({ id: schema.user.id })
		)[0]!.id
    await db
      .insert(schema.password)
      .values({ ...createPassword('kodylovesyou'), userId })
    await db.insert(schema.userImage).values({ ...kodyImages.kodyUser, userId })
    await db
    ...
    await db.insert(schema.roleToUser).values([
    ...
    const noteIds = (
      await db
        .insert(schema.note)
        .values(notesToCreate.map((val) => ({ ...val, ownerId: userId })))
        .returning({ id: schema.note.id })
    ).map(({ id }) => id)
    await db
      .insert(schema.noteImage)
      .values(
        noteImagesToCreate
          .map((val, idx) =>
            val.map((innerVal) => ({ ...innerVal, noteId: noteIds[idx]! })),
          )
          .flat(),
      )
